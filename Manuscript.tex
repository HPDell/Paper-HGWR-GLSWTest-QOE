...
Both robustness scenarios use 200 data sets for the $F$ test and 30 for the effect-wise bootstrap at $a=0.5$ and \gls{icc} 0.4.
...
After excluding superseded pilot output, the reported design comprises 1,050 independently executable tasks, 5,110 outer repetitions, and 10,220 effect-level results.
...
The approximated $F$-test is relatively effective.
Its global-null rejection rates range from 6.0\% to 9.4\%, with three of four effect-by-\gls{icc} estimates above 7\%.
It detects $\gamma_1$ in all usable heterogeneous repetitions.
However, its rejection rate for the constant $\gamma_2$ increases with the heterogeneity of $\gamma_1$.
For $a=0.5$, these false-positive rates are 20.8\% at \gls{icc} 0.1 and 11.2\% at \gls{icc} 0.4; for $a=1$, they rise to 49.5\% and 17.1\%.
The results shows that the approximate $F$ test tends to be sensitive to spatial heterogeneity in both components, resulting in relatively high false-positive rates for spatially constant effects when other effects are heterogeneous.
Considering such a problem is more serious when spatial heterogeneity is stronger, it is possibly caused by spillover effects from $\gamma_1$ because the model employs a uniform bandwidth for all \gls{glsw} effects.
...
Overall, the simulation shows that the $F$-test is effective as a rapid screening tool, as it requires less computation resources than the bootstrap methods and detects heterogeneous effects with high power.
...
The full-model bootstrap increase the rate of false positives for constant effects when other effects are heterogeneous compared with the approximated $F$-test, and it is therefore not recommended for effect-specific inference.

\subsubsection{Robustness and bandwidth diagnostic}

\input{generated/simulation_additional_table.tex}

As shown in the previous section, the approximate $F$ test and the effect-wise bootstrap are more effective methods.
Thus, we further evaluated their robustness to heavy-tailed residuals and strongly unbalanced group sizes.
...
Across the 5,110 reported outer repetitions, 96 (1.88\%) fail to produce usable tests.
For fixed-bandwidth experiments, the failure rates are 0.38\% for the $F$ test, 6.89\% for the full-model bootstrap, and 1.50\% for the effect-wise bootstrap.
...
The median serial elapsed times are 1.44 seconds for the $F$ test, 69.81 seconds for the full-model bootstrap, and 132.67 seconds for the effect-wise bootstrap.
The corresponding 95th percentiles are 3.44, 91.39, and 493.18 seconds.
