...
The case study therefore serves two purposes: it illustrates how a heterogeneity test can guide the choice between GLSW and fixed effects, and it shows how simulation evidence about calibration qualifies the interpretation of mapped educational inequalities.
...
Second, it develops an effect-wise parametric bootstrap and a distribution-based approximate $F$ test for GLSW effects in HGWR and uses Monte Carlo experiments to examine their calibration, power, effect specificity, and computational behaviour.
...
The distribution-based method uses the linear form of the GLSW estimator, the hat matrix $\vS$, and the covariance matrix $\vV$ to approximate the null distribution of an observation-weighted coefficient variance by an $F$ distribution.
...
For the $k$-th GLSW effect, both methods below evaluate the spatial constancy stated in \eqref{equ:glsw-heterogeneity-core-hypothesis}.
The bootstrap obtains an empirical reference distribution by repeated model fitting, while the distribution-based method derives an approximate $F$ distribution from the GLSW estimator.
...
For effect $k$, that group-level variable is moved from the GLSW component to the fixed component, while the other GLSW effects retain their fitted spatial surfaces.
...
Bootstrap responses are generated from the fixed and GLSW components of this null model plus newly simulated zero-mean group random effects and residuals; empirical random-effect predictions are not included in the generating mean.
...
\begin{enumerate}[label=\textbf{Step \arabic*.}, left=0.5em, itemindent=2em]
    ...
    \begin{enumerate}
        ...
        \item Refit the original HGWR specification to $\vy^*_i$, normally holding the bandwidth at its value selected from the observed data;
...
\end{equation}
The two GLSW effects are $\gamma_{1j}=2+a s(u_j)$ and $\gamma_{2j}=2$, where $a\in\{0,0.5,1\}$ controls the heterogeneity of the first effect.
...
The results consequently describe the implemented data-generating and fitting configuration, not calibration under every correctly specified HGWR design.
...
The original specification procedure first fits the remaining province-level variables as GLSW effects and uses the approximate $F$ test in \autoref{sec:test-hgwr-glsw-ftest} to screen each surface.
The youth-palace count is not rejected by this screening test and is therefore retained as a fixed rather than a GLSW effect.
...
The $F$ column gives the approximate test of whether each GLSW effect is spatially homogeneous across the study area; the percentages describe how many province-specific estimates reach each local significance level.
...
All retained GLSW effects reject spatial homogeneity according to the approximate $F$ test at the 5\% level, although the evidence is weaker for \Gini than for most other provincial variables.
The Monte Carlo experiment shows that this approximation can be liberal and can reject a constant effect when another GLSW effect is heterogeneous.
...
Building on this definition, we develop an effect-wise parametric bootstrap and an approximate $F$ test for GLSW effects in HGWR.
...
The approximate $F$ test is much faster, but its global-null rejection rates reach 9.4\%, and its false-positive rate for the constant coefficient rises as the other GLSW effect becomes more heterogeneous.
...
The distinction between testing constancy and testing a zero coefficient remains essential: the former determines whether a GLSW representation is statistically warranted.

The QOE case study demonstrates this role in practice.
The approximate $F$ screening results support GLSW specifications for the retained provincial economic and educational variables but not for the youth-palace count, which is therefore represented by a fixed effect.
