...
For each scenario and repetition, the random seed is determined by the scenario index and replication number, beginning from base seed 260902.
...
All full-model result shards identify \texttt{hgwrr} 0.6.2.9005 and the joint-null \texttt{fullmodel} bootstrap path.
...
Under the complete joint null, the full-model bootstrap rejects the $\gamma_1$ component in 3.5\% of usable repetitions at both ICC values and the $\gamma_2$ component in 3.1\% and 2.6\% at ICC 0.1 and 0.4, respectively.
...
The earlier concern that this procedure was intrinsically too conservative is therefore not supported after generating from the joint spatial-constancy null.

The full-model bootstrap also responds strongly when $\gamma_1$ is heterogeneous.
Its $\gamma_1$ rejection rates are 100\% at ICC 0.1 for both alternatives, 97.9\% at ICC 0.4 when $a=0.5$, and 100\% at ICC 0.4 when $a=1$.
...
Taken together, the full-model bootstrap is well calibrated under the complete joint null and powerful for detecting that the joint null has failed, but it does not localise the heterogeneous effect.
The $F$ approximation also loses effect specificity, while the effect-wise bootstrap provides the most balanced effect-level behaviour in the investigated ICC 0.4 alternatives.

The independent $B=999$ effect-wise confirmation rejects $\gamma_1$ in 1 of 20 null data sets and 20 of 20 strong-alternative data sets, while rejecting $\gamma_2$ in neither setting.
These results are directionally consistent with the primary $B=199$ effect-wise results, but the confidence intervals are wide because only 20 independent data sets are used at each endpoint.
