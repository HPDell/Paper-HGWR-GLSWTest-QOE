...
Local regression models are valuable because they allow a regression relationship to vary across space rather than imposing one global coefficient on every location.
...
For such data, the \acrcite{hgwr} model combines GWR with \acrcite{hlm} and separates group-level spatially weighted (\gls{glsw}), fixed, and sample-level random (\gls{slr}) effects \citep{HuHarris-2024}.
...
At the time of writing, \gls{hgwr} provides estimates of \gls{glsw} surfaces but lacks a formal, effect-specific test of this spatial-homogeneity null hypothesis.
The hybrid parametric and non-parametric structure of the model makes both a reference distribution and a valid resampling scheme less direct than in GWR \citep{HuHarris-2024,HuHarris-2024a}.

The gap also requires a precise definition of the object being tested.
We treat a spatial random variable as a random variable conditional on location and define strong spatial heterogeneity as a location-dependent conditional distribution.
Weak spatial heterogeneity concerns a specified characteristic of that distribution, such as a location-dependent conditional expectation.
...
Existing local-regression methods clarify both the opportunity and the limitation.
For GWR, approximate $F$ tests compare a coefficient surface with a spatially constant relationship \citep{LeungMei-2000}, and residual-based bootstrap procedures test whether particular coefficients can be treated as constant without relying on normal errors \citep{MeiXu-2016,HarrisBrunsdon-2017}.
Moran's I and local indicators of spatial association instead assess spatial dependence, while ANOVA and HLM address grouped variation under their own model structures \citep{Moran-1950,LiCalder-2007,DutilleulLegendre-1993,Goldstein-2011}.
None directly supplies a test for a GLSW surface while preserving the joint fixed, random, and locally weighted components of HGWR.

This paper develops two complementary tests for spatial heterogeneity in GLSW effects.
The primary procedure is an effect-wise parametric bootstrap: for each effect under test, it fits a null model in which that effect is spatially constant, generates responses under the fitted hierarchical error structure, refits the original HGWR specification, and compares the observed and simulated variability of the local estimates.
The second is a distribution-based approximate $F$ test derived from the GLSW smoother and hat matrix.
Both procedures test the same null hypothesis, but the simulation treats the $F$ test as a rapid approximation rather than as an automatic validation of the bootstrap result.

We evaluate the procedures by Monte Carlo experiments with known homogeneous and heterogeneous GLSW effects at 200 fixed group locations.
...
We then use household investment in children's extracurricular courses, an important component of quality-oriented education (QOE) in China, as an empirical illustration.
Household expenditure reflects family circumstances, whereas provincial economic and policy variables are candidates for GLSW effects; the case study shows how heterogeneity testing can inform the choice between GLSW and fixed specifications, without treating the resulting associations as causal effects.
...
Second, it develops an effect-wise parametric bootstrap and a distribution-based approximate $F$ test for GLSW effects in HGWR and evaluates their calibration, power, effect specificity, and computational behaviour.
...
\autoref{sec:hgwr} presents the HGWR model and identifies GLSW effects as the object of inference.
