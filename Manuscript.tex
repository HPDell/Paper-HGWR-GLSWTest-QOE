...
For a calibrated \gls{hgwr} model with $\hat{\vgamma}$ as estimates of \gls{glsw} effects, $\hat{\vbeta}$ as estimates of fixed effects, and $\hat{\vD}$ as the estimated variance--covariance matrix of random effects, the distribution of the combined random-effect and residual term can be expressed as \autoref{equ:glsw-bootvar-error-distribution},
...
\begin{enumerate}[label=\textbf{Step \arabic*.}, left=0.5em, itemindent=2em]
  ...
  \begin{enumerate}
    \item Independently draw new group random effects and residuals according to \autoref{equ:glsw-bootvar-error-distribution}, using $\hat{\vD}_{0k}$ and $\hat{\sigma}_{0k}$, and set $\vy^*_i=\hat{\vy}_{0k}+\ve^*_i$;
  ...
  \end{enumerate}
  \item Calculate the $p$ value as \autoref{equ:glsw-bootvar-pvalue},
