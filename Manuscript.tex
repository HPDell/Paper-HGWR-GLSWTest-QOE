...
\begin{abstract}
%% Text of abstract
Hierarchical and geographically weighted regression (HGWR) models spatial hierarchical data by combining group-level spatially weighted (GLSW), fixed, and sample-level random effects. Although GLSW estimates are allowed to vary geographically, variation in an estimated coefficient surface does not by itself establish spatial heterogeneity because sampling error and spatial autocorrelation may produce similar patterns. We address this inferential gap by defining spatial heterogeneity through location-conditional distributions and by developing bootstrap and approximate $F$ tests for the spatial heterogeneity of GLSW effects. A simulation experiment assesses the HGWR-specific procedures. The $F$ test separates the heterogeneous and constant GLSW effects in the simulated models. The bootstrap detects the heterogeneous effect but yields $p=0.040$ for a constant effect in one specification and $p=0.081$ in an alternative specification, indicating sensitivity near a 5\% threshold. We then apply the framework to household investment in children's extracurricular courses in China, integrating household survey, provincial panel, geographic, and point-of-interest data. The heterogeneity tests guide the allocation of provincial variables to GLSW or fixed effects and reveal that the associations of economic development, living costs, income inequality, university recruitment, education budgets, and the highly educated population with hourly extracurricular-course expenditure vary across provinces. The study therefore demonstrates how formal tests can turn mapped coefficient variation into statistically qualified evidence in an applied HGWR analysis.
\end{abstract}
...
The motivation is to develop a test of spatial heterogeneity for a relatively new method of spatial analysis, which is HGWR. 
However, the discussion of what is meant by spatial heterogeneity is more foundational, and will be of interest beyond the specific case study of HGWR.
...
The hierarchical and geographically weighted regression \citep[HGWR;][]{HuHarris-2024} model is a synthesis of geographically weighted regression \citep[GWR;][]{BrunsdonFotheringham-1996} and hierarchical linear modelling \citep[HLM;][]{Goldstein-2011}.
To handle spatial hierarchical data,
HGWR includes three types of effects: group-level (`upper level') spatially weighted (GLSW) effects, fixed effects, and sample-level (`lower level') random (SLR) effects.
GLSW and SLR effects allow for the possibility of spatial heterogeneity at the group and sample levels, respectively, while fixed effects capture globally constant influences (i.e. effects that do not vary geographically across the study region).
% A key restriction is that GLSW effects are applicable only to group-level variables, whereas SLR effects are restricted to sample-level variables.
% To estimate these effects, HGWR employs a back-fitting algorithm and a maximum likelihood estimator \citep[BFML;][]{ChenLeung-2022}, enabling simultaneous estimation of all effects within the hierarchical structure.

However, at the time of writing, relatively little work has been undertaken to situate the model within a formal framework of statistical inference, particularly with respect to testing the significance of any spatial heterogeneity observed in estimates of GLSW effects.
As a hybrid of parametric and non-parametric methods \citep{HuHarris-2024,HuHarris-2024a}, HGWR lacks a well-established inferential foundation.
The reasons for this are twofold.
When adopting a probability-based approach to inference, the hierarchical structure introduces additional complexity in deriving probability distributions compared to the hypothesis tests of GWR \citep{LeungMei-2000}.
...
As spatial heterogeneity and spatial dependence often happen together, Moran's I and, in particular, local indicators of spatial association \citep[LISA;][]{Anselin-1995} ---
...
When GWR is used to model spatially-varying regression relationships, one may then analyse the spatial heterogeneity in these relationships using either an $F$-test \citep{LeungMei-2000} or a bootstrap test \citep{HarrisBrunsdon-2017}.
For hierarchical or grouped data, analysis of variance (ANOVA) and HLM are feasible methods \citep{DutilleulLegendre-1993,FotheringhamBrunsdon-2010,LignierJarvis-2024,KirkbrideStochl-2015} in addition to caterpillar plots visualising intraclass correlation in data grouped by geographical locations.
...
Methods using Moran's I and LISA assume that spatial heterogeneity arises from autocorrelation, while ANOVA or HLM rely on grouping or repeated observations \citep{HuLu-2022,HuHarris-2024}.
...
Whether concerning random variables or GLSW effects within the HGWR model, equating spatial variation with spatial heterogeneity is not always appropriate.
...
To fill this gap, after introducing the HGWR model, we clarify what constitutes spatial heterogeneity in this study, as well as the specific form of spatial heterogeneity that we aim to test.
...
This definition identifies spatial constancy as the relevant null hypothesis for the GLSW effects in HGWR.
We evaluate that null hypothesis using bootstrap and distribution-based methods, thereby assessing whether the variation in an estimated GLSW effect is greater than would be expected under a spatially constant relationship.
...
A mapped HGWR coefficient can suggest that a provincial association differs geographically, but policy interpretation requires evidence that the variation is greater than would be expected under a spatially constant effect.
The case study therefore serves two purposes: it demonstrates how the proposed tests guide the choice between GLSW and fixed effects, and it shows how inferentially supported heterogeneity changes the substantive interpretation of educational inequalities.
...
Second, it develops residual-bootstrap and distribution-based $F$ tests for GLSW effects in HGWR and uses a simulation to examine whether the procedures recover known homogeneous and heterogeneous effects.
Third, it provides an empirical study in which the tests are integrated into effect specification rather than applied only after estimation, thereby connecting statistical evidence of spatial heterogeneity to household- and province-level correlates of extracurricular education investment.

The remainder of this paper will be organised as follows.
\autoref{sec:hgwr} outlines the mathematical foundations of the HGWR model and identifies the GLSW effects as the object of inference.
...
\autoref{sec:glsw-test} develops bootstrap and distribution-based $F$ tests for GLSW effects and assesses their effectiveness by simulation.
...
Due to the existence of spatial heterogeneity, local-form regression models \citep{FotheringhamBrunsdon-2010} analysing non-stationary relationships are popular with geographers, like GWR \citep{HelbichBrunauer-2014,FanSong-2021,TranPearson-2022} and HLM \citep{WuHou-2011,KirkbrideStochl-2015,WangJiang-2021}.
As a combination of GWR and HLM, the HGWR model overcomes the limitations of the two models, such as the risk of singular matrix in GWR and the inability to model spatial heterogeneity for group-level variables in HLM \citep{HuHarris-2024}.
However, methods for testing the significance of spatial heterogeneity of the non-stationary relationships have been developed for GWR via bootstrapping or $F$-test \citep{LeungMei-2000,MeiXu-2016,HarrisBrunsdon-2017} and HLM \citep{GelmanHill-2006,Goldstein-2011},
while HGWR lacks such methods.

Developing a hypothesis test for spatial heterogeneity in HGWR is challenging due to the model's complexity.
At minimum, it is important to establish that the variation in GLSW effect estimates differs from the variation produced by the random component of the model.
To achieve this goal, starting from reviewing the basis of this model is essential.
This section summarises the definition and estimation of the HGWR model, as the bootstrap method requires resampling according to the hierarchical error structure, and the $F$-test requires the GLSW estimator and its hat matrix.
...
% The HGWR model \citep{HuHarris-2024} integrates geographically weighted regression and hierarchical modelling.
In a two-level HGWR model, variables are classified as either group level or sample level, and their effects can be fixed or varying.
For sample $i$ in group $j$ from a data set of $n$ samples across $m$ groups, the HGWR model can be expressed as \eqref{equ:hgwr-model-pointwise},
...
\end{equation*}
with $h,p,q$ are the number of GLSW, fixed, and SLR effects, respectively.
...
The HGWR model can be expressed in the matrix from, as shown in \eqref{equ:hgwr-model},
...
Each $\vgamma_{j} (j=1,2,\cdots,m)$ in $\vgamma$ is estimated through a GWR-like method by borrowing information from its geographical neighbours, using a kernel function $k$ with a parameter called bandwidth, $b$.
These are specified like the standard GWR methods \citep{ComberBrunsdon-2022}.
...
For the HGWR model described above, the BFML estimator follows this procedure.
...
\item Estimate GLSW effects.
Construct $\vy_g^{(t)}=\vy-\vX\hat{\vbeta}^{(t-1)}$.
Regard $\vZ\vmu$ as a part of random errors.
For $j$-th location $\vu_j$, by applying a GWR-like method, $\hat{\vgamma}$ there can be estimated as \eqref{equ:bfml-glsw-estimator},
...
\item Estimate covariance of SLR effects, $\vD$, and variance of random errors, $\sigma^2$, denoted by $\hat{\vD}^{(t)}$ and $\hat{\sigma}^{(t)}$.
...
The preceding section shows that HGWR allows the GLSW effect $\vgamma_j$ to vary among locations.
...
While methods such as LISA and GWR are available for analysing spatial heterogeneity,
...
To measure the statistical difference, we can apply hypothesis tests such as ANOVA \citep{Gelman-2005} or likelihood ratio test \citep{Woolf-1957}.
...
Testing equality of the complete density at all locations would be difficult in practice and is not necessary for the purpose of HGWR, where the primary interest is whether a conditional relationship changes across space.
...
For the $k$-th GLSW effect in HGWR, this definition leads to the null hypothesis
...
For the $k$-th GLSW effect, denote this statistic by
...
The final part is consequently to determine the distribution of $T_k$ under spatial constancy.
There are two possible routes in this study.
The bootstrap method repeatedly generates samples according to the null-generating mechanism and compares the observed dispersion with the empirical distribution of the resampled dispersions.
The distribution-based method uses the linear form of the GLSW estimator, the hat matrix $\vS$, and the covariance matrix $\vV$ to approximate the null distribution of the dispersion by an $F$ distribution.
Thus, the two methods use the same object and the same null hypothesis; their difference lies in how the reference distribution is obtained.
The following section incorporates the hierarchical error structure and GLSW estimator to construct the corresponding bootstrap and $F$ tests for HGWR.

\section{Testing Spatial Heterogeneity in GLSW Effects} \label{sec:glsw-test}

According to \autoref{sec:heterogeneity-testing-logic}, testing a GLSW effect is to determine whether its variation among locations is significantly greater than the variation generated under a spatially constant effect.
The target is different from the conventional test of whether a coefficient is significantly different from zero.
For the $k$-th GLSW effect, both methods below evaluate the spatial constancy stated in \eqref{equ:glsw-heterogeneity-core-hypothesis} using the variance of its local estimates.
The bootstrap method obtains the reference distribution by repeated model fitting, while the distribution-based method derives an approximate $F$ distribution from the GLSW estimator.

% In this section, we concentrate on designing hypothesis tests for fixed effects and GLSW effects of the HGWR model.
% The significance of deviation from zero is evaluated for both fixed effects and GLSW effects using a $t$-test.
% Additionally, testing spatial heterogeneity is considered for GLSW effects using an $F$-test and a bootstrap test.
...
% For an HGWR model with $p$ fixed effects, $q$ SLR effects, $r$ GLSW effects, $n$ observations, and $m$ groups, the total number of parameters is given by \eqref{equ:hgwr-test-np},
% \begin{equation}\label{equ:hgwr-test-np}
%     \NP = r m + p + \frac{q(q+1)}{2} + 1
% \end{equation}
% However, due to the data borrowing process, the GLSW effect parameters are not fully independent.
% Thus, we estimate the effective number of parameters of GLSW effects as \eqref{equ:hgwr-enp-glsw},
...
The primary objective for the GLSW effects is to determine whether their spatial variation is significant under the definition and null hypothesis established in \autoref{sec:heterogeneity-testing-logic}.
...
\begin{align}\label{equ:hgwr-glsw-boot-hypothesis}
    ...
    \end{split}
\end{align}
Because the GLSW effects are estimated within a hierarchical regression model, their local estimates inherit correlation from data borrowing and the model's error structure.
Both theoretical and experimental findings indicate that directly testing for spatial heterogeneity in GLSW effect estimates results in an excessively high Type I error rate (i.e., incorrectly rejecting the null hypothesis of no spatial heterogeneity), making the test ineffective.
...
For HGWR, experiments show that resampling observations may lead to an insufficient number of samples in some locations, potentially causing model calibration to fail.
Therefore, resampling residuals is a more suitable approach, as recommended by \citet{HarrisBrunsdon-2017}.

For a calibrated HGWR model with $\hat{\vgamma}$ as estimates of GLSW effects, $\hat{\vbeta}$ as estimates of fixed effects, and $\hat{\vD}$ as estimated variance-covariance matrix of random effects, the distribution of the error term (including the random effects), $\ve$ can be expressed as \eqref{equ:glsw-bootvar-error-distribution},
\begin{equation}\label{equ:glsw-bootvar-error-distribution}
    \ve = \vZ\vmu + \vepsilon \sim \vN\left(\vzero, (\vZ\vD\vZ\T+\vI)\sigma^2\right)
\end{equation}
By resampling errors, $\ve^*$, we can generate bootstrapped samples, $\vy^*$, and re-fit an HGWR model with the same settings.
The procedure for testing the significance of spatial heterogeneity in GLSW effect estimates is as follows.
\begin{enumerate}[label=\textbf{Step \arabic*.}, left=0.5em, itemindent=2em]
    \item For the original model, calculate the variance of GLSW effect estimates, $\hat{\vgamma}$, denoted by $S$;
    \item Repeat the following steps $B$ times. In each iteration $i$,
    \begin{enumerate}
        \item Resample a set of errors, $\ve^*_i$, with $\hat{\vD}$ according to \eqref{equ:glsw-bootvar-error-distribution}, and generate bootstrap samples $\vy^*_i = \diag(\vG\hat{\vgamma}) + \vX\hat{\vbeta} + \ve^*_i$;
        \item Fit an HGWR model with $\vy^*_t$ with the same settings to obtain GLSW effect estimates, denoted by $\hat{\vgamma}^*_t$;
        \item Calculate the variance of $\hat{\vgamma}^*_t$, denoted by $S^*_i$;
    \end{enumerate}
    \item Calculate the $p$ value as \eqref{equ:glsw-bootvar-pvalue},
    \begin{equation}\label{equ:glsw-bootvar-pvalue}
        p = \frac{\sum_{i=1}^B 1_{S^*_i > S}}{B}
    \end{equation}
    and compare $p$ with the pre-defined significance level $\alpha$.
\end{enumerate}

Although HGWR is more efficient than GWR and MGWR \citep{HuHarris-2024}, the entire procedure remains time-intensive.
In this case, a recommended value of $B$ is $999$, as it achieves a favourable balance between reducing the error rate and controlling computational cost.

\subsection{Distribution-based method}\label{sec:test-hgwr-glsw-ftest}

As an alternative and cross-validation to the bootstrap method,
we can also design a probability-based hypothesis test for GLSW effects following the method proposed by \citet{LeungMei-2000}.
...
\begin{equation}\label{equ:hypothesis-glsw-ftest}
    ...
    \end{split}
\end{equation}
For the $k$-th GLSW effect, we examine its variance under the null hypothesis, $V_k^2$, as \eqref{equ:glsw-ftest-variance},
...
\mathop{\mathrm{var}}\left( \frac{V_{k}^2}{\sigma^{2}} \right) & = 2\mathrm{tr}\left[ \frac{1}{n} \mybm{\Gamma}^{\mathrm{T}} \left( \mybm{I}-\frac{1}{n}\mybm{J} \right) \mybm{\Gamma}\mybm{V} \right]^2 .
\end{align}
...
\mathop{\mathrm{var}}\left( \frac{\mathrm{RSS}_{g}}{\sigma^{2}} \right) & = 2\mathrm{tr}\left[ (\mybm{I}-\mybm{S})^{\mathrm{T}}(\mybm{I}-\mybm{S})\mybm{V} \right]^2 \quad .
\end{align}
...
\end{equation}
For a given confidence level $\alpha$, the upper quantile is denoted by $F_\alpha$.
We can reject the null hypothesis if $F \geqslant F_\alpha$.

% \subsection{Sample-level random effects}

% Like the GLSW effects, an important question in testing the significance of SLR effects is whether they are heterogeneous between groups instead of whether they are significantly different from zero.
% For SLR effects, which are similar to the random effects in HLM, we use the likelihood ratio test typically \citep{MyersMontgomery-2012}.
...
To verify the effectiveness of bootstrapping and distribution-based tests for HGWR,
we designed a simulation with a spatial data set of 625 locations, $\vu_j=(u_j,v_j)$ ($j=1,2,\cdots,625$).
At each location, $j$, a random number ($n_j$) of samples are randomly generated, with $n_j \sim U(20,50)$.
Eight independent variables, $\vg_1$, $\vg_2$, $\vg_3$, $\vx_1$, $\vx_2$, $\vx_3$, $\vz_1$, $\vz_2$, were randomly generated across these points following a multivariate standard normal distribution.
To simulate group-level spatially-related variables, the mean values of $\vg_1$ and $\vg_2$ were substituted at each location.
Four effects, $\vgamma_1$, $\vgamma_2$, $\vbeta_1$, and $\vmu_1$, corresponding to the variables $\vg_1$, $\vg_2$, $\vx_1$, and $\vz_1$, together with an intercept, $\valpha_0$, were generated at each location $\vu_j$ according to \eqref{equ:sim-hgwrtest-coef},
\begin{align}\label{equ:sim-hgwrtest-coef}
    \begin{split}
        \alpha_0(u,v) & = 2 + \eta_0 \\
        \gamma_1(u,v) & = \frac{1}{324}\left[36 - \left(6 - \frac{u}{2}\right)^2\right]^2 \\
        \gamma_2(u,v) & = 2 \\
        \beta_1(u,v) & = 2 \\
        \mu_1(u,v) & = \eta_1
    \end{split}
\end{align}
where $\eta_0,\eta_1 \sim N(2,0.2^2)$.
These values are depicted in \autoref{fig:sim-hgwrtest-coef}.
Finally, we generate the dependent variable, $\vy$, as \eqref{equ:sim-hgwrtest-response},
\begin{equation}\label{equ:sim-hgwrtest-response}
    y_{ij} = \alpha_{0j} + g_{1ij} \cdot \gamma_{1j} + g_{2ij}\cdot\gamma_{2j} + x_{1ij} \cdot \beta_1 + z_{1ij} \cdot \mu_{1j} + \epsilon_{ij}
\end{equation}
where $\vepsilon \sim N(0, 1)$.

\begin{figure}[tbp]
    \centering
    \includegraphics[width=\linewidth]{fig/fig_HGWRTest_coef}
    \caption{Coefficient values in the simulation.}
    \label{fig:sim-hgwrtest-coef}
\end{figure}

% \begin{table}[tbp]
%     \centering\small
%     \caption{Statistical summary of model 1.}
%     \label{tab:sim-hgwrtest-summary-model1}
%     \begin{tabular}{lrrrrrr}\toprule
%     & $\beta_1$ & $\beta_2$ & $\beta_3$ & $\gamma_1$ & $\gamma_2$ & $\mu_1$ \\
%     \midrule
%     Est.            & 2.000   & -0.001 & -0.000 &        &       & 1.952 \\
%     S.D.            & 0.007   & 0.007  &  0.007 &        &       & 0.366 \\
%     $t$             & 284.268 & 0.122  & 0.054  &        &       &       \\
%     Mean Est.       &         &        &        & 1.851  & 2.013 &       \\
%     Mean S.D.       &         &        &        & 0.046  & 0.044 &       \\
%     $F$             &         &        &        & 22.686 & 0.719 &       \\
%     $p$             & 0.000   & 0.903  & 0.957  & 0.000  & 0.998 &       \\
%     $p$ (bootstrap) &         &        &        & 0.000  & 0.245 &       \\
%     \bottomrule
%     \end{tabular}
% \end{table}
...
\begin{table}
  \centering\small
  \caption{Statistical summary of the HGWR models in the simulation.}
  \label{tab:sim-hgwrtest-summary-model}
  \begin{tabular}{lrrr|rrr}
  & \multicolumn{3}{c}{\textbf{Model 1}} & \multicolumn{3}{c}{\textbf{Model 2}} \\\toprule
  \textbf{\makecell{Fixed\\effects}} & \makecell[r]{Est.\\(S.D.)}    & \makecell[r]{$t$-value\\($p$-value)} &     & \makecell[r]{Est.\\(S.D.)}    & \makecell[r]{$t$-value\\($p$-value)} &  \\\midrule
  $\beta_1$                          & \makecell[r]{ 2.000\\(0.007)} & \makecell[r]{284.268***\\(0.000)}    &     & \makecell[r]{1.999\\(0.007)}  & \makecell[r]{284.327***\\(0.000)}    &  \\
  $\beta_2$                          & \makecell[r]{-0.001\\(0.007)} & \makecell[r]{  0.122\\(0.903)}       &     & \makecell[r]{     \\       }  & \makecell[r]{       \\       }       &  \\
  $\beta_3$                          & \makecell[r]{-0.000\\(0.007)} & \makecell[r]{  0.054\\(0.957)}       &     & \makecell[r]{     \\       }  & \makecell[r]{       \\       }       &  \\
  $\gamma_1$                         &                               &                                      &     & \makecell[r]{1.663\\(0.176)}  & \makecell[r]{9.441***\\(0.0000)}     &  \\
  \bottomrule
  \toprule
  \textbf{\makecell{GLSW\\effects}} & \makecell[r]{$\overline{\text{Est.}}$\\($\overline{\text{S.D.}}$)} & \makecell[r]{$F$-value\\($p$-value)} & \makecell[r]{Bootstrap\\$p$-value} & \makecell[r]{$\overline{\text{Est.}}$\\($\overline{\text{S.D.}}$)} & \makecell[r]{$F$-value\\($p$-value)} & \makecell[r]{Bootstrap\\$p$-value} \\\midrule
  $\gamma_1$                        & \makecell[r]{1.851\\(0.046)}                                       & \makecell[r]{22.686***\\(0.000)}     & 0.000***                           &                                                                    &                                      &                                    \\
  $\gamma_2$                        & \makecell[r]{2.013\\(0.044)}                                       & \makecell[r]{ 0.998\\(0.245)}        & 0.040*                             & \makecell[r]{2.005\\(0.082)}                                       & \makecell[r]{0.637\\(0.999)}         & 0.081                              \\
  \bottomrule
  \multicolumn{7}{l}{Significance levels: * $p < 0.05$, ** $p < 0.01$, *** $p < 0.001$.}
  \end{tabular}
\end{table}

We fitted two models on this data, named Model 1 and Model 2.
For sample $i$ in group $j$, the two models are defined as \eqref{equ:simulation-model1} and \eqref{equ:simulation-model2},
\begin{align}
    \label{equ:simulation-model1}
    y_{ij} &= \alpha_{0j} + \gamma_{1j}g_{1ij} + \gamma_{2j}g_{2ij} + \beta_{1}x_{1ij} + \beta_{2}x_{2ij} + \beta_{3}x_{3ij} + \mu_{1j}z_{1ij} + \epsilon_{ij} \\
    \label{equ:simulation-model2}
    y_{ij} &= \alpha_{0j} + g_{2ij} \gamma_{2j} + g_{1ij} \gamma_{1} + x_{1ij} \beta_1 + z_{1ij} \mu_{1j} + \epsilon_{ij}
\end{align}
where $\epsilon_{ij} \sim N(0, \sigma_1^2)$.
For Model 1, we expect $\beta_2$ and $\beta_3$ to be insignificantly different from zero, while $\beta_1$ should be significantly different from zero; and $\gamma_{2j}$ should show no significant spatial variation.
For Model 2, we expect that $\beta_1$ and $\gamma_{1}$ to be significantly different from zero;
while $\gamma_{2j}$ should not display significant spatial heterogeneity.
After fitting the model, we performed the significance test.
The results are presented in \autoref{tab:sim-hgwrtest-summary-model}.
As anticipated, $\hat{\beta}_2$ and $\hat{\beta}_3$ were not significantly different from zero in Model 1.
Spatial heterogeneity of $\hat{\gamma}_2$ was tested to be insignificant in both models.
Moreover, $\hat{\beta}_1$ was confirmed to be significantly different from zero in both models, while $\hat{\gamma}_1$ showed significant spatial heterogeneity in Model 1.
The $F$-test separates the heterogeneous $\gamma_1$ surface from the constant $\gamma_2$ effect in both specifications.
The bootstrap test detects the heterogeneous $\gamma_1$ surface, but its result for the constant $\gamma_2$ effect is sensitive to specification and threshold: $p=0.040$ in Model 1 and $p=0.081$ in Model 2.
Thus, the simulation supports the proposed inferential distinction while also showing that the bootstrap decision should not be treated as uniformly calibrated from this single design.
\section{Case study: spatial inequalities in extracurricular education investment in China}\label{sec:qoe-case}

The simulations establish whether the proposed procedures recover known data-generating structures.
This section examines their role in an applied HGWR analysis, where the assignment of a coefficient to the GLSW or fixed component affects both model complexity and substantive interpretation.
...
These multiple levels make the application suitable for HGWR, but they also make it important to distinguish a genuinely spatially heterogeneous provincial association from an estimated surface that varies only because of sampling uncertainty.
...
Province-level covariates are candidates for GLSW effects because their associations with family expenditure may vary geographically.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{llll|llll}
    ...
    \GDPPrimary             & GLSW & Yes & Yes & \CultCentCountTotal     & Fixed & Yes & Yes \\
    \Engel                  & GLSW & Yes & Yes & \TotalIncome            & Fixed & Yes & Yes \\
    \CPI                    & GLSW & Yes & Yes & \TotalSpending          & Fixed & Yes & Yes \\
    \Gini                   & GLSW & Yes & Yes & \Asset                  & Fixed & Yes & Yes \\
    \UniversityRecruitRatio & GLSW & Yes & No  & \Debt                   & Fixed & Yes & Yes \\
    \GeneralEduBudget       & GLSW & Yes & Yes & \Age                    & Fixed & Yes & Yes \\
    \HighEducated           & GLSW & Yes & Yes & \HighEdu                & Fixed & No  & No  \\
    \RegionUrban            & SLR  & No  & No  & \GenderMale             & Fixed & No  & No  \\
    \bottomrule
  \end{tabular}
\end{table}

The specification is not based on the visual variability of coefficient maps alone.
We first fit the remaining province-level variables as GLSW effects and use the spatial heterogeneity test in \autoref{sec:glsw-test} to assess each surface.
The youth-palace count does not exhibit significant spatial heterogeneity and is therefore retained as a fixed rather than a GLSW effect.
...
Finally, an HLM with forward selection identifies \RegionUrban as an SLR effect.
...
The $F$ column tests whether each GLSW effect is spatially homogeneous across the study area; the percentages describe how many province-specific estimates reach each local significance level.
...
All retained GLSW effects reject spatial homogeneity at the 5\% level, although the evidence is weaker for \Gini than for most other provincial variables.

\begin{table}[tbp]
  \centering\scriptsize
  \caption{Summary of the main QOE HGWR model. For GLSW effects, the $F$ statistic tests spatial homogeneity; percentages are the proportions of province-specific estimates in each significance category.}
...
\begin{figure}[tbp]
  ...
  \caption{Province-specific estimates of the GLSW effects and the SLR effect for urban residence in the main QOE model. The global $F$ tests in \autoref{tab:qoe-model-summary}, rather than visual variation alone, establish evidence against spatial homogeneity.}
...
The SLR estimate for urban residence is positive in nearly all provinces and is especially large in Gansu and Chongqing.
...
These policy implications are associational: the HGWR analysis identifies where estimated relationships differ, not the causal effect of changing a budget or economic indicator.
...
Fourth, spatial collinearity may affect the allocation and interpretation of GLSW and fixed effects.
...
This article develops an inferential framework for spatial heterogeneity in HGWR.
...
Building on this definition, we develop residual-bootstrap and approximate $F$ tests for GLSW effects in HGWR.
The simulation shows that the $F$ procedure distinguishes a genuinely varying coefficient from a spatially constant coefficient in both fitted specifications, while conventional tests continue to address whether fixed effects differ from zero.
The bootstrap detects the genuinely varying coefficient, but its $p$-values of $0.040$ and $0.081$ for the constant coefficient across the two specifications reveal sensitivity near conventional thresholds.
The distinction between testing constancy and testing a zero coefficient is essential: the former determines whether a GLSW representation is statistically warranted.

The QOE case study demonstrates this role in practice.
The heterogeneity tests support GLSW specifications for the retained provincial economic and educational variables but not for the youth-palace count, which is therefore represented by a fixed effect.
The fitted model provides evidence that the associations of primary-sector GDP, living costs, income inequality, university recruitment, public education budgets, and the highly educated population with hourly extracurricular-course expenditure vary geographically.
...
The effect of non-i.i.d. errors on the bootstrap procedure requires further theoretical and simulation study beyond the structures considered here.
Bootstrap accuracy also increases computational cost, particularly for large data sets and repeated bandwidth optimisation, whereas the $F$ approximation depends on distributional and smoother-matrix assumptions.
...
Future work should examine effect-size measures, multiplicity across many candidate effects, direct resampling of hierarchical observations, sensitivity to spatial collinearity, and extensions to more than two levels.
...
The $F$-test for GLSW effects introduced in Section~\ref{sec:test-hgwr-glsw-ftest} involves matrices $\vV$, $\vS$, and $\vGamma$ with $n \times n$ elements, which can become computationally intensive due to their large size.
