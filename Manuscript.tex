...
Three testing procedure are tested in this simulation: the approximate $F$ test, the full-model bootstrap, and the effect-wise bootstrap.
...
Under the alternatives, its rejection rates for the heterogeneous $\gamma_1$ are 2.0\% and 0\% at ICC 0.1 and 23.0\% and 13.3\% at ICC 0.4 for $a=0.5$ and 1, respectively, while the constant $\gamma_2$ is never rejected.
This low and non-monotone response to increasing $a$ is expected to differ from ordinary power because the generator retains the fitted heterogeneous surface rather than imposing $H_{0k}$.
It shows that the full-model reference reproduces much of the fitted dispersion and is highly conservative as a decision rule; these values must not be interpreted as effect-wise null calibration or power.
...
The median serial elapsed times are 1.44 seconds for the $F$ test, 44.23 seconds for the full-model bootstrap, and 132.67 seconds for the effect-wise bootstrap.
The corresponding 95th percentiles are 3.44, 61.19, and 493.18 seconds.
...
Moreover, because the full-model generator retains the unrestricted fitted surfaces, its rejection rates describe agreement with that fitted-model reference rather than effect-wise Type I error or power.
