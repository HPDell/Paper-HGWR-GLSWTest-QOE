...
The fitted full-model bootstrap is much more conservative and responds non-monotonically under the alternatives, so it is informative as a reproduction-based model reference but not as an effect-wise null test.
...
In the software used here, this procedure is implemented by the existing fitted-model (\texttt{legacy}) bootstrap mode; no alternative full-model implementation is substituted in the experiments.
...
Let $\hat{q}$ denote the covariance multiplier used by this fitted-model bootstrap implementation.
The stored residual scale is $\hat{\sigma}$, and the implementation sets $\hat{q}=\hat{\sigma}$ rather than $\hat{\sigma}^{2}$.
Its combined simulated error distribution is therefore
\begin{equation}\label{equ:glsw-bootvar-error-distribution}
  \ve^{*}_{i}=\vZ\vmu^{*}_{i}+\vepsilon^{*}_{i}
  \sim \vN\left(
    \vzero,
    (\vZ\hat{\vD}\vZ\T+\vI)\hat{q}
  \right),
\end{equation}
where the observed coordinates, covariates, group membership, and group sample sizes are held fixed.
This implementation-specific scale convention is kept unchanged so that the simulation evaluates the full-model procedure actually available to users.
...
The full-model bootstrap uses the fitted model to generate bootstrap responses and refits the same model to each response, producing two effect-specific tests from each repetition.
...
All full-model result shards identify \texttt{hgwrr} 0.6.2.9003 and the \texttt{legacy}/\texttt{fitted} bootstrap path.
...
\subsubsection{Calibration, power, effect specificity, and model-based reference}
...
The full-model bootstrap gives zero rejections for both effects in both global-null scenarios; the upper limits of the four 95\% intervals are approximately 1.5\%.
Under the alternatives, its rejection rates for the heterogeneous $\gamma_1$ are 1.0\% and 0\% at ICC 0.1 and 22.0\% and 13.3\% at ICC 0.4 for $a=0.5$ and 1, respectively, while the constant $\gamma_2$ is never rejected.
This low and non-monotone response to increasing $a$ does not estimate ordinary power because the generator retains the fitted heterogeneous surface rather than imposing $H_{0k}$.
It shows that the full-model reference reproduces much of the fitted dispersion and is highly conservative as a decision rule; its implementation-specific simulation scale is an additional reason not to interpret these values as effect-wise null calibration or power.
At ICC 0.4, its rejection of heterogeneous $\gamma_1$ is 74.9 and 86.7 percentage points below the effect-wise bootstrap for $a=0.5$ and 1, respectively.
...
Taken together, the full-model reference rarely flags either effect, the $F$ approximation always flags heterogeneous $\gamma_1$ but also frequently flags constant $\gamma_2$, and the effect-wise bootstrap provides the most balanced behaviour in the investigated ICC 0.4 alternatives.
...
Across the 5,110 reported outer repetitions, 38 (0.74\%) fail to produce usable tests.
For fixed-bandwidth experiments, the failure rates are 0.38\% for the $F$ test, 0.44\% for the full-model bootstrap, and 1.50\% for the effect-wise bootstrap.
...
The median serial elapsed times are 1.44 seconds for the $F$ test, 86.99 seconds for the full-model bootstrap, and 132.67 seconds for the effect-wise bootstrap.
The corresponding 95th percentiles are 3.44, 122.58, and 493.18 seconds.
At the median, the effect-wise procedure takes about 1.5 times as long as the full-model procedure because it requires a separate null fit and bootstrap sequence for each tested effect.
...
Moreover, because the full-model generator retains the unrestricted fitted surfaces and uses the fitted-mode scale convention defined above, its rejection rates describe agreement with that implemented fitted-model reference rather than effect-wise Type I error or power.
...
The full-model bootstrap, evaluated over all six core scenarios, produces much lower and non-monotone rejection rates because it retains the fitted heterogeneous surfaces in its generating mean; it is therefore reported as a model-based reference rather than an effect-wise null test.
